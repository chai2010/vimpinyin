...
function s:InputContext.GetCursorLeftInput() dict
	...
	if self.GetCharsNum() == 1 | return "v" | endif
...
function! VimPinyin_VEnEditer_MakeQuery(inCtx, outCtx)
	let chars = a:inCtx.GetInputWord()
	let list = VimPinyin_VEnEngine_Search(chars)
...
" 显示输入栏
function! VimPinyin_VEnEditer_ShowEnInputBar(inCtx, outCtx)
...
" 处理输入
function! VimPinyin_VEnEditer_ProcessInput(keycode_)
	...
	while 1
		...
		if  bInputChanged
			...
			let v = VimPinyin_VEnEditer_MakeQuery(inCtx, outCtx)
		endif
		...
		" 显示输入面板
		call VimPinyin_VEnEditer_ShowEnInputBar(inCtx, outCtx)
		" 获取新的字符
		let keycode = getchar()
...
" 清空状态
function! VimPinyin_VEnEditer_Reset()
endfunction
