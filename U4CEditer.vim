...
let s:InputContext = {}
" 原始输入字符
let s:InputContext.in_chars = [ 'u', '1', '0', '8', '0', ]
" 当前光标位置
...
function s:InputContext.IsInputKey(keycode) dict
	if self.GetCharsNum() <= 0
		if a:keycode == char2nr('u') | return 1 | endif
	elseif self.GetCharsNum() < 5
		if VimPinyin_Common_IsDigit(a:keycode) | return 1 | endif
		if a:keycode == char2nr('?') | return 1 | endif
	elseif self.GetCharsNum() == 5
		if a:keycode == char2nr('?') | return 1 | endif
		return 0
		if VimPinyin_Common_IsDigit(a:keycode) | return 1 | endif
		if VimPinyin_Common_IsLower(a:keycode) | return 1 | endif
	else
		return 0
	endif
	return 0
...
function s:InputContext.PushChar(keycode) dict
	...
	if self.GetCharsNum() > 5 | return 0 | endif
...
function s:InputContext.GetCursorLeftInput() dict
	...
	if self.GetCharsNum() == 1 | return "u" | endif
...
function! VimPinyin_U4CEditer_MakeQuery(inCtx, outCtx)
	let chars = a:inCtx.GetInputWord()
	let list = VimPinyin_U4CEngine_Search(chars)
...
" 显示输入栏
function! VimPinyin_U4CEditer_ShowEnInputBar(inCtx, outCtx)
	...
	"------------------------------------------------------
	" [四角] vhe|l >> 1.[hel] 2.help 3.held 4.helpful 5.hello
	
	execute 'echohl Title | echon "["'
	execute 'echohl Title | echon "四角"'
	execute 'echohl Title | echon "] "'
	...
	for i in range(len(out_list))
		...
		if a:inCtx.GetCharsNum() < 5
			if i == cursor_pos
				execute 'echohl LineNr | echon "' . nr2char(char2nr('a')+i) . '."'
				execute 'echohl None | echon "["'
			...
			if i == cursor_pos
				execute 'echohl LineNr | echon "' . nr2char(char2nr('1')+i) . '."'
				execute 'echohl None | echon "["'
				execute 'echohl Label | echon "' . out_list[i] . '"'
				execute 'echohl None | echon "] "'
			else
...
" 处理输入
function! VimPinyin_U4CEditer_ProcessInput(keycode_)
	...
	while 1
		...
		if  bInputChanged
			...
			let v = VimPinyin_U4CEditer_MakeQuery(inCtx, outCtx)
		endif
		...
		" 显示输入面板
		call VimPinyin_U4CEditer_ShowEnInputBar(inCtx, outCtx)
		" 获取新的字符
		let keycode = getchar()
...
" 清空状态
function! VimPinyin_U4CEditer_Reset()
endfunction
